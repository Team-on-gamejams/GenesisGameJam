...
public class RMF_RadialMenuElement : MonoBehaviour {
	...
	public void setParentMenuLable(string l) {
		...
		if (parentRM.textLabel != null) {
			parentRM.sunParent.SetActive(priceSun > 0);
			parentRM.waterParent.SetActive(priceWater > 0);

			parentRM.textLabel.text = l;
			parentRM.priceSunField.text = priceSun.ToString();
			parentRM.priceWaterField.text = priceWater.ToString();
